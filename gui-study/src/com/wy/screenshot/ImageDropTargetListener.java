...
public class ImageDropTargetListener extends DropTargetAdapter {
	...
	public void drop(DropTargetDropEvent event) {
		...
		if (imgFiles.size() == 1) {
			...
			mainFrame.getSavepathTF()
					.setText(filename.substring(0, filename.lastIndexOf("\\")) + Common.getNewFileorDirName(filename));
		} else if (imgFiles.size() > 1) {
			mainFrame.getFilepathTF().setText(filename.substring(0, filename.lastIndexOf("\\")));
			mainFrame.getSavepathTF().setText(filename.substring(0, filename.lastIndexOf("\\")) + "\\加水印后");
	...
	private BufferedImage addWatermark(File file) {
		...
		BufferedImage buffImg = ImageTool.watermark(file, font, fontColor, toward, mark, alpha, scale);
		return buffImg;
	}
	...
	private ArrayList<FileModel> getBufferImages(List<File> imgFileList) {
