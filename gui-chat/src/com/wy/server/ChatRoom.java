...
public class ChatRoom extends Thread implements ActionListener {
	...
	public void run() {
		...
		Vector<Customer> user_exit = new Vector<>();
		try {
			for (;;) {
				Socket toServer;
				...
				for (int i = intMessageCounter; i < messobj.chat.size(); i++) {
					...
					if (temp.chatUser.equals(strLoginName)) {
						if (temp.chatToUser.equals(strLoginName)) {
							...
							{
								temp_message = "【你】对【" + temp.chatToUser + "】" + "说："
										+ temp.chatMessage + "\n";
							} else {
								temp_message = "【你】悄悄对【" + temp.chatToUser + "】" + "说："
										+ temp.chatMessage + "\n";
							}
						}
					} else {
						if (temp.chatToUser.equals(strLoginName)) {
							if (!temp.whisper) // 不是悄悄话
							{
								temp_message = "【" + temp.chatUser + "】对【你】" + "说："
										+ temp.chatMessage + "\n";
							} else {
								temp_message = "【" + temp.chatUser + "】悄悄对【你】" + "说："
										+ temp.chatMessage + "\n";
							}
						} else {
				...
				if (messobj.userOnLine.size() < intUserTotal) {
					for (int b = 0; b < user_exit.size(); b++) {
						...
						for (int c = 0; c < messobj.userOnLine.size(); c++) {
							...
							if (tempstr.equals(
									((Customer) messobj.userOnLine.elementAt(c)).custName)) {
								isFound = true;
								break;
							}
						}
	...
	public void saveMessage() {
		try {
			FileOutputStream fileoutput = new FileOutputStream(this.strLoginName + "_message.txt",
					true);
