...
public class Connection extends Thread {
	 ...
	 * 注册用户列表
	 */
	private static Vector<RegisterCustomer> vList = new Vector<>();
	 ...
	 * 创建与客户端通讯
	 */
	public Connection(ServerFrame frame, Socket client, Vector<Customer> u, Vector<Chat> c) {
		netClient = client;
		userOnline = u;
		userChat = c;
		sFrame = frame;
	...
	public void serverLogin() {
		...
		try {
			...
			vList = (Vector<RegisterCustomer>) objInput1.readObject();
			int find = 0; // 查找判断标志
			for (int i = 0; i < vList.size(); i++) {
				RegisterCustomer reg = (RegisterCustomer) vList.elementAt(i);
	...
	public void serverRegiste() {
		...
		try (ObjectInputStream objInput = new ObjectInputStream(new FileInputStream(fList));) {
			int flag = 0; // 是否重名判断标志
			RegisterCustomer clientMessage = (RegisterCustomer) obj;
			if (fList.length() != 0) {
				// 判断是否是第一个注册用户
				vList = (Vector<RegisterCustomer>) objInput.readObject();
				// 判断是否有重名
				for (int i = 0; i < vList.size(); i++) {
					RegisterCustomer reg = (RegisterCustomer) vList.elementAt(i);
