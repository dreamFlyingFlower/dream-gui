...
/**
 * 数据部分,一个内部数组来表示页面上的具体情况
 * @author ParadiseWY
 * @date 2019年10月7日 下午11:10:07
 */
@Getter
@Setter
public class Map {
	public static int LEFTCOUNT = Props.ROWS * Props.COLUMNS;

	/**
	 * 让其最外层的数据不显示,可以解决边框消除不掉的情况
	 */
	private int[][] map = new int[Props.ROWS + 2][Props.COLUMNS + 2];
	 ...
	 */
	private void init() {
		int[] tempArr = new int[Props.ROWS * Props.COLUMNS];
		...
		for (int i = 1; i < Props.ROWS + 1; i++) {
			for (int j = 1; j < Props.COLUMNS + 1; j++) {
				this.map[i][j] = tempArr[(i - 1) * Props.COLUMNS + (j - 1)];
			}
		}
	}
	...
	public boolean twoConner(ArrayPoint p1, ArrayPoint p2) {
		...
		// 右
		count = Props.COLUMNS + 2 - p1.j;
		for (int col = p1.j; col < Props.COLUMNS + 2; col++) {
			temp = new ArrayPoint(p1.i, col, map[p1.i][col]);
		...
		// 下
		for (int row = p1.i + 1; row < Props.ROWS + 2; row++) {
			temp = new ArrayPoint(row, p1.j, map[row][p1.j]);
			if ((temp.value == 0) && oneLineWithoutValue(p1, temp)) {
				if (oneConnerWithoutValue(temp, p2)) {
					return true;
